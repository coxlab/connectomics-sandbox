    ...
    def transform(self, X, with_fit=False):
        ...
        if lnorm_size is not None:
            ...
            X2 = np.dstack((
                ...
                #lcdnorm3(X2, lnorm_size, contrast=True, divisive=False),
            ))
...
def main():
    ...
    print 'model...'
    from skimage.filter import median_filter, tv_denoise
    from scipy import ndimage
    #trn_X = median_filter(trn_X)
    #trn_X = ndimage.morphology.grey_dilation(trn_X, (2, 2))
    ...
    if 0:
        ...
        #tst_X = ndimage.morphology.grey_dilation(tst_X, (2, 2))
        Y_pred = mdl1.predict(tst_X)[..., 0]
        Y_pred = median_filter(Y_pred, radius=3)
        #Y_pred = tv_denoise(Y_pred, weight=10)
