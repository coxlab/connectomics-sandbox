...
def main():
    ...
    def rand_func(alpha, *args):
        ...
        # unpack args
        _w_t, _b_t, _w_best, _b_best, _X, _Y_true = args
        key = joblib.hash("".join([joblib.hash(e) for e in args] + [joblib.hash(alpha)]))
        if key not in rand_func_cache:
            # get prediction
            mdl1.w = _w_best + alpha * _w_t
            mdl1.b = _b_best + alpha * _b_t
            _Y_pred = mdl1.predict(_X)[..., 0]
            _Y_pred = median_filter(_Y_pred, radius=3)
            # get rand error
            offset = 32
    ...
    for t in xrange(N_ITERATIONS):
        ...
        if not WARM_RESTART:
            mdl1.w = None
            mdl1.b = None
        mdl1.fit(tmp_X, tmp_Y_true)
        w_t = mdl1.w.copy()
        b_t = mdl1.b.copy()
        alpha_best = optimize.brent(
            rand_func,
            args=(w_t, b_t, w_best, b_best, tmp_X, tmp_Y_true),
            tol=tol)
        w_best = (1. - lr) * w_best + lr * alpha_best * w_t
        b_best = (1. - lr) * b_best + lr * alpha_best * b_t
