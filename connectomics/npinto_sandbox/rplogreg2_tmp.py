...
class RPLogReg2(object):
    ...
    def fit(self, X, Y):
        ...
        #t_loss = (1. - w_neg) * tensor.sum((t_yb * (tensor.maximum(0, 1 - t_M - m_pos) ** 2.))) / (t_yb>0).sum()
        #t_loss += w_neg * tensor.sum((1 - t_yb) * tensor.maximum(0, 1 - t_M - m_neg) ** 2.) / ((1-t_yb)>0).sum()
...
def main():
    ...
    def genXY(trn_X, trn_Y, i):

        if rng.binomial(1, .5):
            print 'rotate...'
            trn_X = ndimage.rotate(trn_X, i * 90, prefilter=False, order=0)
            trn_Y = ndimage.rotate(trn_Y, i * 90, prefilter=False, order=0)
    ...
    rand_func_cache = {}
    def rand_func(alpha, *args):
        print '=' * 80
        print 'proposed alpha:', alpha
        # unpack args
        _e_Y_pred, _tmp_Y_pred, _Y_true = args
        key = joblib.hash("".join([joblib.hash(e) for e in args] + [joblib.hash(alpha)]))
        if key not in rand_func_cache:
            # get prediction
            _Y_pred = _e_Y_pred + alpha * _tmp_Y_pred
            # get rand error
            _Y_pred -= _Y_pred.min()
            _Y_pred /= _Y_pred.max()
            offset = 32
            _Y_pred = _Y_pred[offset:-offset, offset:-offset]
            _Y_true = _Y_true[offset:-offset, offset:-offset]
            rd = rand_error(_Y_true, _Y_pred)
            print 'rand...'
            rand_func_cache[key] = rd
        else:
            rd = rand_func_cache[key]
        print '-' * 80
        print 'alpha=%s, rd=%s' % (alpha, rd)
        return rd

    # --
    def predict_ensemble(mdl_l, alpha_l, X):
        Y_pred = 1.
        for m, a in zip(mdl_l, alpha_l):
            print a, m
            Y_pred += a * m.predict(X)[..., 0]
        return Y_pred
    ...
    for t in xrange(N_ITERATIONS):
        ...
        # gen data
        tmp_X, tmp_Y_true = genXY(trn_X, trn_Y, t)

        # gen new model and its prediction
        mdl = RPLogReg2(rf_size=rf_size,
                        lnorm_size=lnorm_size,
                        n_filters=n_filters,
                        lbfgs_params=lbfgs_params,
                        learning=learning,
                       )
        mdl.fit(tmp_X, tmp_Y_true)
        tmp_Y_pred = mdl.predict(tmp_X)[..., 0]

        # gen ensemble prediction
        e_Y_pred = predict_ensemble(mdl_l, alpha_l, tmp_X)

        # find best alpha
        alpha_best = optimize.brent(
            rand_func,
            args=(e_Y_pred, tmp_Y_pred, tmp_Y_true),
            tol=tol)

        mdl_l += [mdl]
        alpha_l += [alpha_best]

        # evaluate on other image
        print '-' * 80
        print 'on tst image'
        Y_pred = predict_ensemble(mdl_l, alpha_l, tst_X)
        Y_true = tst_Y.copy()
        print 'pe =', pearson(Y_true.ravel(), Y_pred.ravel())
        Y_pred -= Y_pred.min()
        Y_pred /= Y_pred.max()
